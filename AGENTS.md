# Kritical.Lens.ALDependencyMatrix — AGENTS.md
...
|---|---|---|---|---|
| `AGENTS.md` | Kritical.Lens.ALDependencyMatrix — AGENTS.md | 2026-08-01 | R1-root-entry | — |
| `README.md` | Kritical.Lens.ALDependencyMatrix | 2026-07-04 | R1-root-entry | — |
...
Invoke-KriticalLensSweep -Root 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.Lens.ALDependencyMatrix'
...
Invoke-KriticalLensLanguageSweep -Root 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.Lens.ALDependencyMatrix'

# both families, plus a combined receipt
Invoke-KriticalLensEstateSweep -Roots 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.Lens.ALDependencyMatrix'

# end-of-turn gate (parser + PsLint auto-fix + PSScriptAnalyzer + Lens + corpus top-up)
& "$lens\scripts\Invoke-KritEndOfTurnGate.ps1" -Repo 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.Lens.ALDependencyMatrix'
```
...
| Raw git corpus (every commit, every blob) | SQL Server `.\SQLEXPRESS` database `KritLens_Kritical_Lens_ALDependencyMatrix` (tables `lens.git_corpus_version` / `lens.git_corpus_blob`) | `Invoke-KritLensGitCorpusFullIngest` |
...
```powershell
& "$lens\scripts\Test-KritLensCorpusFreshness.ps1" -Only 'Kritical.Lens.ALDependencyMatrix'
```
...
| --- | --- | --- |
| `Kritical.Lens.PSGraph` | YES (always runs) | 8 PowerShell file(s) — PS call graph, HOT/MEDIUM/COLD ranking |
...
| `Language.JSON` | YES — 4 file(s) | node CLI present |
...
| Markdown | 4 |
| PowerShell | 8 |
| **all tracked-ish files (excl. .git/node_modules/bin/obj/dist)** | **21** |
...
| `capabilities.json` | Declared capability manifest |
| `src/meta.json` | Module metadata |
...
- `docs/AST-SCALAR-COLLAPSE-SWEEP-20260725.md`
